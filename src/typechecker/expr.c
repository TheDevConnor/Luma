...
AstNode *typecheck_binary_expr(AstNode *expr, Scope *scope, ArenaAllocator *arena) {
  ...
  if (op >= BINOP_EQ && op <= BINOP_GE) {
    ...
    if (match == TYPE_MATCH_NONE) {
      tc_error_help(expr, "Type Error",
             "Comparison operands must be of compatible types",
...
AstNode *typecheck_call_expr(AstNode *expr, Scope *scope,
  ...
  if (!func_symbol) {
  tc_error(expr, "Type Error", "Undefined function '%s'", func_name ? func_name : "unknown");
  return NULL;
  }
  ...
  if (!arg_type) {
    tc_error(expr, "Type Error",
         "Failed to type-check argument %zu in call to '%s'",
         i + 1, func_name);
    return NULL;
  }
...
AstNode *typecheck_member_expr(AstNode *expr, Scope *scope,
  ...
  tc_error(expr, "Type Error", "'%s' has no member '%s'", base_name, member_name);
  }
  return NULL;
}
...
AstNode *typecheck_addr_expr(AstNode *expr, Scope *scope,
  ...
  if (!base_type) {
  tc_error(expr, "Type Error",
      "Failed to type-check address-of expression");
  return NULL;
  }
  AstNode *pointer_type =
...
AstNode *typecheck_cast_expr(AstNode *expr, Scope *scope,
  ...
  if (!castee_type) {
  tc_error(expr, "Type Error",
      "Cannot determine type of cast operand");
  return NULL;
  }
