...
AstNode *typecheck_member_expr(AstNode *expr, Scope *scope,
  ...
  if (base_symbol && base_symbol->type) {
    ...
    printf("DEBUG: Member access - base '%s' has type %p (category %d, type %d)\n", 
           base_name, (void*)base_type, base_type->category, base_type->type);
    
    // Handle pointer dereference: if we have a pointer to struct, automatically dereference it
    if (base_type->type == AST_TYPE_POINTER) {
      ...
      if (pointee && pointee->type == AST_TYPE_BASIC) {
        ...
        Symbol *struct_symbol = scope_lookup(scope, pointee->type_data.basic.name);
        if (struct_symbol && struct_symbol->type && struct_symbol->type->type == AST_TYPE_STRUCT) {
    ...
    if (base_type->type == AST_TYPE_BASIC) {
      ...
      Symbol *struct_symbol = scope_lookup(scope, base_type->type_data.basic.name);
      if (struct_symbol && struct_symbol->type && struct_symbol->type->type == AST_TYPE_STRUCT) {
