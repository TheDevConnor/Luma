...
Stmt *const_stmt(Parser *parser, bool is_public, bool returns_ownership,
    ...
    case TOK_FN:
        return fn_stmt(parser, name, is_public, returns_ownership,
                       takes_ownership);
    case TOK_STRUCT:
...
Stmt *fn_stmt(Parser *parser, const char *name, bool is_public,
    ...
    return create_func_decl_stmt(
        ...
        returns_ownership, takes_ownership, body, line, col);
}
...
Stmt *struct_stmt(Parser *parser, const char *name, bool is_public) {
    ...
    if (!growable_array_init(&public_fields, parser->arena, 4,
                             sizeof(Stmt *)) ||
        !growable_array_init(&private_fields, parser->arena, 4,
                             sizeof(Stmt *))) {
    ...
    while (p_has_tokens(parser) && p_current(parser).type_ != TOK_RBRACE) {
        ...
        if (tok == TOK_PUBLIC || tok == TOK_PRIVATE) {
            ...
            p_advance(parser);
            p_consume(parser, TOK_COLON,
                      "Expected ':' after visibility keyword");
            continue;
        }
        ...
        if (p_current(parser).type_ == TOK_EQUAL) {
            ...
            field_function =
                fn_stmt(parser, field_name, public_member, false, false);
        } else {
        ...
        Stmt *field_decl = create_field_decl_stmt(
            parser->arena, field_name, field_type, field_function,
    ...
    return create_struct_decl_stmt(
        ...
        (Stmt **)private_fields.data, private_fields.count, is_public, line,
        col);
}
...
Stmt *block_stmt(Parser *parser) {
    ...
    while (p_has_tokens(parser) && p_current(parser).type_ != TOK_RBRACE) {
        ...
        if (!slot) {
            fprintf(stderr,
                    "Out of memory while growing block statement array\n");
            return NULL;
        }
...
Stmt *if_stmt(Parser *parser) {
    ...
    p_consume(parser, p_current(parser).type_,
              "Expected 'if' or 'elif' keyword");
    ...
    Stmt **elif_stmts = (Stmt **)arena_alloc(parser->arena, sizeof(Stmt *) * 4,
                                             alignof(Stmt *));
    int elif_count = 0;
...
Stmt *infinite_loop_stmt(Parser *parser, int line, int col) {
    ...
    if (!body) {
        parser_error(parser, "Syntax Error", __FILE__,
                     "Expected block statement", line, col, 1);
        return NULL;
    }
...
Stmt *for_loop_stmt(Parser *parser, int line, int col) {
    ...
    while (p_has_tokens(parser) && p_current(parser).type_ != TOK_RBRACKET) {
        ...
        if (!slot) {
            fprintf(stderr,
                    "Out of memory while growing loop initializers array\n");
            return NULL;
        }
    ...
    if (p_current(parser).type_ == TOK_COLON) {
        ...
        p_consume(parser, TOK_LPAREN,
                  "Expected '(' after ':' in loop statement");
    ...
    return create_for_loop_stmt(parser->arena, (AstNode **)intializers.data,
                                intializers.count, condition,
                                optional_condition, body, line, col);
}
...
Stmt *print_stmt(Parser *parser, bool ln) {
    ...
    while (p_has_tokens(parser) && p_current(parser).type_ != TOK_RPAREN) {
        ...
        if (!slot) {
            fprintf(stderr,
                    "Out of memory while growing print expressions array\n");
            return NULL;
        }
...
Stmt *switch_stmt(Parser *parser) {
    ...
    while (p_has_tokens(parser) && p_current(parser).type_ != TOK_RBRACE) {
        ...
        if (p_current(parser).type_ == TOK_IDENTIFIER &&
            ...
            default_case = create_default_stmt(parser->arena, default_body,
                                               case_line, case_col);
            continue;
        }
        ...
        if (!growable_array_init(&case_values, parser->arena, 4,
                                 sizeof(Expr *))) {
        ...
        *value_slot = case_expr;
        ...
        while (p_current(parser).type_ == TOK_COMMA) {
            ...
            if (!additional_slot) {
                fprintf(stderr,
                        "Out of memory while growing case values array\n");
                return NULL;
            }
...
Stmt *impl_stmt(Parser *parser) {
    ...
    while (p_has_tokens(parser) && p_current(parser).type_ != TOK_RBRACKET) {
        ...
        char **name_identifier =
            (char **)growable_array_push(&function_name_list);
        Type **type_specifier =
            (Type **)growable_array_push(&function_name_types);

        if (!name_identifier || !type_specifier) {
            fprintf(stderr,
                    "Out of memory while growing function list for impl\n");
            return NULL;
        }
    ...
    while (p_has_tokens(parser) && p_current(parser).type_ != TOK_RBRACKET) {
        ...
        char **struct_identifier =
            (char **)growable_array_push(&struct_name_list);

        if (!struct_identifier) {
            fprintf(stderr,
                    "Out of memory while growing struct list for impl\n");
            return NULL;
        }
