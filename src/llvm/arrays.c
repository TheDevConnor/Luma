...
LLVMValueRef codegen_expr_array_index(CodeGenContext *ctx, AstNode *node) {
  ...
  if (object_kind == LLVMArrayTypeKind) {
    ...
    // FIXED: Use the array_alloca (pointer) instead of object (array value)
    ...
    // Pointer to array or allocated array
    // Check if the object is a symbol we can look up for type info
    if (node->expr.index.object->type == AST_EXPR_IDENTIFIER) {
      ...
      if (sym && !sym->is_function) {
        ...
        if (sym_kind == LLVMArrayTypeKind) {
          ...
          // FIXED: Use the symbol's value (which is already a pointer to the
          // array)
          LLVMValueRef element_ptr =
    ...
    // Fall back to regular pointer indexing
...
// Enhanced array assignment to handle array elements
LLVMValueRef codegen_expr_array_assignment(CodeGenContext *ctx, AstNode *node) {
  ...
  if (target->type != AST_EXPR_INDEX) {
    return NULL; // Not an array assignment
  }
  ...
  if (object_kind == LLVMArrayTypeKind) {
    ...
    if (target->expr.index.object->type == AST_EXPR_IDENTIFIER) {
      ...
      if (sym && !sym->is_function) {
        ...
        fprintf(stderr, "Error: Array variable not found for assignment\n");
        return NULL;
      }
    } else {
    ...
    // Use existing pointer indexing logic from codegen_expr_assignment
