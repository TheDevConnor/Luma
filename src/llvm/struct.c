...
LLVMValueRef codegen_stmt_struct(CodeGenContext *ctx, AstNode *node) {
    ...
    for (size_t i = 0; i < total_members; i++) {
        printf("  - %s: %s (%s)\n", struct_info->field_names[i], 
               "type", struct_info->field_is_public[i] ? "public" : "private");
    }
    #endif
...
LLVMValueRef codegen_expr_struct_access(CodeGenContext *ctx, AstNode *node) {
    ...
    if (object->type == AST_EXPR_IDENTIFIER) {
        // Access like: x->field or x.field (where x is a variable)
        ...
        // Check if this is a pointer to struct (like Person*)
        if (LLVMGetTypeKind(symbol_type) == LLVMPointerTypeKind) {
            // Find the struct type by checking which struct has this field
            for (StructInfo *info = ctx->struct_types; info; info = info->next) {
    ...
    if (!struct_info || !struct_ptr) {
        fprintf(stderr, "Error: Could not determine struct type for member access '%s'\n", field_name);
        return NULL;
    }
    ...
    LLVMValueRef field_ptr = LLVMBuildStructGEP2(
        ctx->builder, struct_info->llvm_type, struct_ptr, field_index, "field_ptr");
...
LLVMValueRef codegen_struct_literal(CodeGenContext *ctx, const char *struct_name, 
    ...
    if (field_count != struct_info->field_count) {
        fprintf(stderr, "Error: Struct literal field count mismatch for '%s': expected %zu, got %zu\n",
    ...
    if (all_constant) {
        ...
        LLVMValueRef struct_alloca = LLVMBuildAlloca(ctx->builder, struct_info->llvm_type, "struct_lit");
