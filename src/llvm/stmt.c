...
LLVMValueRef codegen_stmt_return(CodeGenContext *ctx, AstNode *node) {
  ...
  // Check if we have function-level deferred statements
  if (ctx->deferred_statements) {
    // For function-level defers, we should branch to cleanup blocks, not execute inline
    // The cleanup blocks are generated by generate_cleanup_blocks() in the function handler
    ...
    // Instead of executing deferred statements inline, branch to cleanup
    // The cleanup blocks should have been created by the function handler
    
    // Find the cleanup entry point - this should exist if we have deferred statements
    LLVMBasicBlockRef cleanup_entry = NULL;
    
    // Look for the cleanup_entry block in the current function
    ...
    if (func) {
      ...
      while (bb) {
        ...
        if (bb_name && strcmp(bb_name, "cleanup_entry") == 0) {
          cleanup_entry = bb;
          break;
        }
    ...
    if (cleanup_entry) {
      // Branch to cleanup instead of executing inline
      LLVMBuildBr(ctx->builder, cleanup_entry);
      return NULL; // Don't generate the actual return here
    } else {
      // Fallback: no cleanup blocks found, just return normally
      // This shouldn't happen if generate_cleanup_blocks() was called properly
      fprintf(stderr, "Warning: Deferred statements found but no cleanup blocks generated\n");
